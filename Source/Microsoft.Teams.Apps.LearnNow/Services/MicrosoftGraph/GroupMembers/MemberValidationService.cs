...
namespace Microsoft.Teams.Apps.LearnNow.Services.MicrosoftGraph.GroupMembers
{
    ...
    public class MemberValidationService : IMemberValidationService
    {
        ...
        /// <param name="securityGroupOptions">Instance of IOptions to read security group data from application configuration.</param>
        ...
        public async Task<bool> ValidateMemberAsync(string userAadObjectId, string authorizationHeaders)
        {
            ...
            return await this.groupMembersService.GetGroupMemberAsync(this.securityGroupOptions.Value.TeacherSecurityGroupId, userAadObjectId, accessToken);
        }
        ...
        public async Task<bool> ValidateAdminAsync(string userAadObjectId, string authorizationHeaders)
        {
            var accessToken = await this.accessTokenHelper.GetAccessTokenAsync(userAadObjectId, authorizationHeaders);
            if (string.IsNullOrEmpty(accessToken))
            {
            ...
            // Check whether current user is a member of administrators's security group.
            return await this.groupMembersService.GetGroupMemberAsync(this.securityGroupOptions.Value.AdminGroupId, userAadObjectId, accessToken);
        }
    }
}