...
class EditTag extends React.Component<WithTranslation, IEditTagState> {
    ...
    */
    ...
    /**
    *Returns text component containing error message for failed tag field validation
    *@param {Boolean} isTagTitleExists Indicates whether tag title already exists or not
    *@param {Boolean} showGenericErrorMessage Indicates whether to show generic error message if any unhandled exception occurs
    */
    private getErrorMessage = (isTagTitleExists: boolean, showGenericErrorMessage: boolean) => {
        if (isTagTitleExists) {
            return (<ErrorMessage errorMessage="tagAlreadyExistsValidationMessage" />);
